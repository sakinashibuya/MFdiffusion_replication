...
In this document, we aim to understand the codes used in [Banerjee et al. (2013), "The Diffusion of Microfinance"](https://science.sciencemag.org/content/341/6144/1236498).
...
1. For a given parameter $\theta$ and each village $r$, compute $d(r, \theta) = \frac{1}{S} \sum_s m_{sim, r}(s, \theta) - m_{emp, r}$, where $m_{sim, r}(s, \theta)$ is the $s$'th simulated moment, $m_{emp, r}$ is the empirical moment, and $S$ is the number of simulations
...
The R script for the replication of the results with endorsement effect is found here (not prepared yet).
...
Since we use an R package called `igraph` to derive the statistics, we do not use the functions defined in these Matlab files.
...
Here we define a function to calculate moments, given who take up microfinance.
This function is used to calculate simulated moments and empirical moments.
When calculating empirical moments, we use the data of take-ups of each household.
On the other hand, for each village we simulate which households take up microfinance, which is used when calculating simulated moments.
...
## Define a function to compute the deviation of the empirical moments from the simulated ones (function `divergence_model`)

We define a function to compute $d(r, \theta)$ for a village $r$ and a given paramter $\theta$.
...
The point estimates are different from the ones in the paper, which could be because of the simulation.
