...
author: 'Mizuhiro Suzuki, Kazuki Motohashi, Sakina Shibuya'
date: "`r Sys.Date()`"
output: pdf_document
---
...
Each circle indicates a household. Blue circles are households in the biggest cluster, while and pink circles are those which are not.
...
(**I coundn't find an explanation as to which variable indicates what. Maybe I should try harder to find it.**) <!-- I also couldn't find this info. -->
...
On the other hand, we simulate take-ups of microfinance by households for each village, which is used then used to calculate simulated moments.
...
The point estimates are slightly different from the ones in the paper, which could be because of the simulation procedure.
